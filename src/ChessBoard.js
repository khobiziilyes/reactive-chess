...
export default function ChessBoard() {
  const { squares, isLightTurn } = useSelector(
    (state) => state.squares.present
  );
      ...
      <tbody>
        ...
        {finalSquares.map((chessRow) => (
          <tr key={`tr-${chessRow[0].rowId}`}>
            <td className="chess-text">
              {chessRow[0].rowCode} - {chessRow[0].rowId}
            </td>

            {chessRow.map((chessSquare) => (
              <ChessSquareComponent
                chessSquare={chessSquare}
                key={`SQUARE-${chessSquare.columnCode}${chessSquare.rowId}`}
              ></ChessSquareComponent>
            ))}
