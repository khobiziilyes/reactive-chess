...
export default function setWhoWon(state, lastMoverIsLight) {
    ...
    const afterPossible = produce(squares, draft => {
        const oppositeSquares = filterSquares(draft, _ => _.piece?.isLightColor === !lastMoverIsLight);
    ...
    const oppositeKing = findSquare(squares, _ => _.piece?.isLightColor === !lastMoverIsLight && _.piece.name === 'king');
